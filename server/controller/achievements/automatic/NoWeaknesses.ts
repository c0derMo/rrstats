...
export class NoWeaknesses implements AutomaticAchievement {
    name = "No Weaknesses";
    description = [
    ...
    async recalculateAll(
        matches: Match[],
        achievements: Record<string, Achievement<AchievementData>>,
    ): Promise<void> {
    ...
    private checkCondition(
        ...
        if (Math.min(...winrates) >= 0.5 && achievement.achievedAt[0] <= 0) {
