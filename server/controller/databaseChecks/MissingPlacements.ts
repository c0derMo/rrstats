...
export class MissingPlacements implements DatabaseCheck {
    info: CheckInfo = {
        ...
        description: "Checks for players that have matches in a tournament, but no assigned placement",
    };
        ...
        for (const player of players) {
            ...
            for (const competition of playedComps) {
                if (!placements.includes(competition.competition)) {
                    issues.push(`Player ${player.primaryName} is missing placement for competition ${competition.competition}`);
                }
            }
        }
