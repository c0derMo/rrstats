...
export class InvalidScores implements DatabaseCheck {
    ...
    async execute(): Promise<CheckResult> {
        ...
        for (const match of matches) {
            ...
            if (totalScore % 2 !== 0 && totalScore !== 1) {
                issues.push(`Match ${this.formatMatch(match, uuidsToPlayers)} has abnormal total score: ${match.playerOneScore} - ${match.playerTwoScore}`);
            }
            ...
            if (p1MapScore !== match.playerOneScore) {
                errors.push(`${uuidsToPlayers[match.playerOne]} of match ${this.formatMatch(match, uuidsToPlayers)} has a score of ${match.playerOneScore} but won ${p1MapScore} worth of maps`);
            }
            if (p2MapScore !== match.playerTwoScore) {
                errors.push(`${uuidsToPlayers[match.playerTwo]} of match ${this.formatMatch(match, uuidsToPlayers)} has a score of ${match.playerTwoScore} but won ${p2MapScore} worth of maps`);
            }
        }
    ...
    private formatMatch(match: Match, uuidsToPlayers: Record<string, string>) {
        return `${uuidsToPlayers[match.playerOne]} vs ${uuidsToPlayers[match.playerTwo]} (${match.competition}, ${match.round})`;
    }
}