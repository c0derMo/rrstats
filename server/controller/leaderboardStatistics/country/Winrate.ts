...
export class CountryWinrate implements LeaderboardCountryStatistic {
    ...
    calculate(
        ...
        const winratePerCountry: DefaultedMap<DefaultedMap<{ wins: number; matches: number }>> = new DefaultedMap(() => new DefaultedMap(() => { return { wins: 0, matches: 0 } }));
        for (const match of matches) {
            ...
            if (nationalityOne != null) {
                winratePerCountry.get(nationalityOne).get(match.playerOne).matches += 1;
            }
            if (nationalityTwo != null && nationalityOne !== nationalityTwo) {
                winratePerCountry.get(nationalityTwo).get(match.playerTwo).matches += 1;
            }

            if (match.playerOneScore > match.playerTwoScore) {
                if (nationalityOne != null) {
                    winratePerCountry.get(nationalityOne).get(match.playerOne).wins += 1;
                }
            } else if (match.playerTwoScore > match.playerOneScore) {
                if (nationalityTwo != null) {
                    winratePerCountry.get(nationalityTwo).get(match.playerTwo).wins += 1;
                }
            } else {
                if (nationalityOne != null) {
                    winratePerCountry.get(nationalityOne).get(match.playerOne).wins += 0.5;
                }
                if (nationalityTwo != null) {
                    winratePerCountry.get(nationalityTwo).get(match.playerTwo).wins += 0.5;
                }
            }
        }
        ...
        for (const country in winratePerCountry.getAll()) {
            const totalMatches = winratePerCountry.get(country).mapAll((k, v) => v.matches).reduce((prev, cur) => prev + cur);
            const totalWins = winratePerCountry.get(country).mapAll((k, v) => v.wins).reduce((prev, cur) => prev + cur);
            ...
            result.push({
                ...
                players: winratePerCountry.get(country).mapAll((player, value) => {
                    return {
                        player: player,
                        displayScore: ((value.wins / value.matches) * 100).toFixed(2) + "%",
