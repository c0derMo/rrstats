...
export default class DiscordAuthIntegration {
    ...
    private static getRedirectURI(): string {
        return encodeURIComponent(`${this.PUBLIC_ORIGIN}/api/auth/discord_callback`);
    }
    ...
    static async handleLogin(sessionId: string, discordCallbackCode: string, discordState: string): Promise<User | null> {
        if (sessionId !== discordState) {
            throw new Error("State and session don't align");
        }
        
        const data = `client_id=${this.DISCORD_ID}` +
        ...
        let token: string;
        try {
            const tokenRequest = await $fetch<{access_token: string}>("https://discord.com/api/oauth2/token", { method: 'POST', body: data, headers: { 'Content-Type': 'application/x-www-form-urlencoded' } });
        ...
        try {
            const userReqeust = await $fetch<DiscordUser>("https://discord.com/api/v10/users/@me", { headers: { Authorization: `Bearer ${token}` } });
