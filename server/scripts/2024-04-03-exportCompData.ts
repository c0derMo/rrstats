...
function formatSpin(s: Spin) {
    return s.targetConditions.map((target) => {
        ...
        if (target.complications.length > 0) {
            targetS += ` (${target.complications.map((comp) => comp.name).join(",")})`;
        }
...
async function main() {
    ...
    const mappedMatches = matches.map((match) => {
        ...
        for (const mapIdxS in match.playedMaps) {
            ...
            result[`map${mapIdx+1}`] = getMap(match.playedMaps[mapIdx].map)!.abbreviation;
            result[`spinMap${mapIdx+1}`] = match.playedMaps[mapIdx].spin != null ? formatSpin(match.playedMaps[mapIdx].spin as Spin) : "";
