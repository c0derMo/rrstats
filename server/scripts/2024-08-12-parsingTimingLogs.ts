...
async function main() {
    ...
    for (const line of timingLines) {
        ...
        if (timings[functionName] == null) {
            ...
            timings[functionName].longestDuration = Math.max(timings[functionName].longestDuration, time);
            timings[functionName].totalDuration += time;
        }
    }

    const namePadding = Math.max(8, Object.keys(timings).map((t) => t.length).reduce((prev, cur) => Math.max(prev, cur), 0));
    const numCalledPadding = Math.max(10, Object.values(timings).map((t) => String(t.timesCalled).length).reduce((prev, cur) => Math.max(prev, cur), 0));
    const longestDurationPadding = Math.max(1, Object.values(timings).map((t) => String(t.longestDuration).length).reduce((prev, cur) => Math.max(prev, cur), 0)+2);
    const totalDurationPadding = Math.max(5, Object.values(timings).map((t) => String(t.totalDuration).length).reduce((prev, cur) => Math.max(prev, cur), 0)+2);
    const averageDurationPadding = Math.max(5, Object.values(timings).map((t) => String(t.totalDuration / t.timesCalled).length).reduce((prev, cur) => Math.max(prev, cur), 0)+2);
    ...
    if (sortingBy.toLowerCase() === "avg") {
        result.sort((a, b) => {
            return (a.totalDuration / a.timesCalled) - (b.totalDuration / b.timesCalled);
        })
    ...
    console.log(`${"Function".padEnd(namePadding)} | ${"Num Called".padEnd(numCalledPadding)} | ${"Total".padEnd(totalDurationPadding)} | ${"Average".padEnd(averageDurationPadding)} | ${"Max".padEnd(longestDurationPadding)}`);
    for (const line of result) {
        const averageLength = line.totalDuration / line.timesCalled;
        console.log(`${line.name.padEnd(namePadding)} | ${String(line.timesCalled).padEnd(numCalledPadding)} | ${String(line.totalDuration).padEnd(totalDurationPadding-2)}ms | ${String(averageLength).padEnd(averageDurationPadding-2)}ms | ${String(line.longestDuration).padEnd(longestDurationPadding-2)}ms`);
    }
}
