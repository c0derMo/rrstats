...
async function main() {
    ...
    const competition = await Competition.find({ where: { tag: Not(In(ignoredComps)), officialCompetition: true }, order: { startingTimestamp: "DESC" } });
    console.log(`Loaded ${competition.length} competitions.`);
    const placements = await CompetitionPlacement.find({ where: { competition: In(competition.map(c => c.tag)) }});
    ...
    const playerLookupMap = MapperService.createStringMapFromList(players, "uuid", "primaryName");

    const eloStats = new PlayerElo().calculate(players, matches, placements, competition);
