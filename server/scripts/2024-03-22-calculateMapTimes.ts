...
async function run() {
    ...
    const matches = await Match.find({ where: { competition: In(["RR13", "RRWC2023"]) } });
    ...
    for (const map of getAllMaps()) {
        ...
        const avg = Math.round(stats.reduce((prev, cur) => prev+cur, 0) / stats.length);
        const med = getQuartile(stats, 0.5);
        const lowerQuart = getQuartile(stats, 0.25);
        const upperQuart = getQuartile(stats, 0.75);
        console.log(`${getMap(map)!.name.padEnd(16)} | ${formatS(avg)} | ${formatS(med)} | ${formatS(lowerQuart)} | ${formatS(upperQuart)}`)
    }
