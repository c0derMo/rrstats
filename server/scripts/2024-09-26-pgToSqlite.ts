...
async function main() {
    consola.box(
        "This script pushes the contents of an postgres database file to a SQLite database using TypeORM.",
    );
    consola.log(
    ...
    consola.warn("Continuing will drop the sqlite database.");
    if (
    ...
    const sqliteComps = await pgDatasource.getManager().find(Competition);
    await sqliteDatasource.getManager().save(sqliteComps);
    consola.success(
    ...
    const sqlitePlacements = await pgDatasource
        .getManager()
        .find(CompetitionPlacement);
    await sqliteDatasource.getManager().save(sqlitePlacements);
    consola.success(
    ...
    const sqliteMaps = await pgDatasource.getManager().find(PlayedMap);
    ...
    await sqliteDatasource.getManager().save(sqliteMaps, { chunk: 1000 });
    consola.success(`Successfully migrated ${sqliteMaps.length} played maps.`);

    // Step 4: Matches
    consola.start("Migrating matches (4/8)...");
    const sqliteMatches = await pgDatasource.getManager().find(Match);
    await sqliteDatasource.getManager().save(sqliteMatches);
    consola.success(`Successfully migrated ${sqliteMatches.length} matches.`);

    // Step 5: Players
    consola.start("Migrating players (5/8)...");
    const sqlitePlayers = await pgDatasource.getManager().find(Player);
    await sqliteDatasource.getManager().save(sqlitePlayers);
    ...
    const sqliteGenRecords = await pgDatasource
        .getManager()
        .find(GenericRecord);
    await sqliteDatasource.getManager().save(sqliteGenRecords);
    consola.success(
    ...
    const sqliteMapRecords = await pgDatasource.getManager().find(MapRecord);
    await sqliteDatasource.getManager().save(sqliteMapRecords);
    consola.success(
    ...
    const sqliteUsers = await pgDatasource.getManager().find(User);
    await sqliteDatasource.getManager().save(sqliteUsers);
