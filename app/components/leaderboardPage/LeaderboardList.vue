<template>
    <div class="flex flex-col">
        ...
        <template v-for="(subcategory, idx) of subcategories" :key="idx">
            <div
            ...
            >
                <FontAwesomeIcon
                    ...
                    :class="{
                        'rotate-180': expandedSubcategories.has(
                            subcategory.name,
                        ),
                    }"
...
const subcategories = computed<
    { name: string; leaderboards: LeaderboardTableDefinition[] }[]
>(() => {
    const subcategories = new Set<string>();
    const leaderboardsPerCategory: Record<
        string,
        LeaderboardTableDefinition[]
    > = {};
    ...
    return [...subcategories].map((subcategory) => {
        return {
            name: subcategory,
            leaderboards: leaderboardsPerCategory[subcategory] ?? [],
        };
    });
});
