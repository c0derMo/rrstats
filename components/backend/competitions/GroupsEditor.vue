<template>
    <div class="flex flex-col gap-3">
        <TextInputComponent placeholder="Matches between players in groups" v-model="settings.matchesBetweenPlayers" type="number" @update:model-value="checkAndEmit()" />
        <TextInputComponent placeholder="Max points per match" v-model="settings.maxPointsPerMatch" type="number" @update:model-value="checkAndEmit()" />
        ...
        <div class="flex flex-row gap-5">
            ...
            <ButtonComponent @click="removeGroup()"><FontAwesomeIcon
                    :icon="['fa', 'trash']"
                    class="text-red-500"
                /></ButtonComponent>
            <ButtonComponent @click="addGroup()"><FontAwesomeIcon
                    :icon="['fa', 'plus']"
        ...
        <div v-if="selectedGroupIndex != null" class="flex flex-col gap-3">
            <TextInputComponent v-model="settings.groups[selectedGroupIndex].groupName" placeholder="Group name" @update:model-value="checkAndEmit()" />
            <TextInputComponent v-model="settings.groups[selectedGroupIndex].advancingPlayers" placeholder="Number of advancing players" type="number" @update:model-value="checkAndEmit()" />

            <h1>Players in group:

                <ButtonComponent class="float-right" @click="addPlayer()"><FontAwesomeIcon
                    :icon="['fa', 'plus']"
                    class="text-green-500"
                /></ButtonComponent>
            </h1>
            <ul>
                <li v-for="player, idx in settings.groups[selectedGroupIndex].players" class="flex flex-row gap-3">
                    <TextInputComponent v-model="playersInGroups[selectedGroupIndex][idx]" class="flex-grow" :error="playersIncorrect[selectedGroupIndex][idx]" @update:model-value="(val) => tryLookupPlayer(val, selectedGroupIndex!, idx)" />
                    <ButtonComponent @click="removePlayer(idx)"><FontAwesomeIcon
                    :icon="['fa', 'trash']"
                    class="text-red-500"
                /></ButtonComponent>
                </li>
            ...
            <ol>
                <li v-for="_, idx in settings.groups[selectedGroupIndex].players" class="flex flex-row">
                    <span class="w-10 mt-2">{{ idx+1 }}:</span>
                    <DropdownComponent :items="positionOverrideDropdown" v-model="positionOverrides[selectedGroupIndex][idx]" @update:model-value="checkAndEmit()" />
                </li>
            </ol>
        </div>
    </div>
        ...
        for (const group of settings.value.groups) {
            ...
            for (let i = 0; i < group.players.length; i++) {
                if (group.positionOverrides !== undefined) {
                    const playerName = uuidsToPlayer[group.positionOverrides[i]];
...
function checkAndEmit() {
    ...
    for (let i = 0; i < settings.value.groups.length; i++) {
        ...
        for (let j = 0; j < settings.value.groups[i].players.length; j++) {
            settings.value.groups[i].players[j] = playerNameToUUIDs.value[playersInGroups.value[i][j]];

            if (positionOverrides.value[i][j] !== '-- none --') {
                settings.value.groups[i].positionOverrides[j] = playerNameToUUIDs.value[positionOverrides.value[i][j]];
            }
        }
    }
