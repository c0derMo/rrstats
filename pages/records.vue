<template>
    <div class="flex flex-col gap-3 mx-10">
        ...
        <div class="italic text-center">
            Records are timed using the spin-timer, starting at the arrival of the spin, ending at pressing "Done" on the Roulette page.<br>
            Multi-map records are calculated by adding up all spin-timers of all maps.
        </div>
        
        <GenericRecordTable :records="genericRecords" :players="records?.players" :matches="records?.matches" />
        
        <h3 class="text-2xl">Map-Records</h3>

        <MapRecordTable :records="mapRecords" :players="records?.players" :matches="records?.matches" />
        ...
        <GenericRecordTable :records="retiredRecords" :players="records?.players" :matches="records?.matches" />
    </div>
</template>
...
const genericRecords = computed(() => {
    return records.value?.generic.filter(m => !isRetiredRecord(m.record)) ?? [];
})
