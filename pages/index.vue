<template>
    <CompetitionBackground :competitions="competitions?.filter(m => m.backgroundImage !== undefined && m.backgroundImage !== null).map(m => m.backgroundImage) as string[]"/>

    <CardComponent class="overflow-x-visible">
        <h1 class="text-lg">RRStats v3</h1>
        <div class="flex flex-row gap-5 my-2">
            <DropdownComponent :items="competitionsDropdown" v-model="selectedTournament" />
            <NuxtLink to="/records">
